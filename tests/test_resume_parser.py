...
def save_resume_data(resume_data, file_path):
    ...
    try:
        if not resume_data["sections"]:
            raise ValueError("Extracted resume data is empty.")
...
def main():
    ...
    input_path = r"C:\Users\troy_\Downloads\Tailored Resume Process\S3\resume_tdunkley.json"
    output_path = r"C:\Users\troy_\Downloads\Tailored Resume Process\S3\test_resume_output.json"

    try:
        # Load resume data
        resume_data = load_json(input_path)
        logger.info(f"Loaded resume data: {json.dumps(resume_data, indent=4)}")
        
        # Save extracted resume data to a JSON file
        save_json(resume_data, output_path)
        logger.info(f"Resume data successfully saved to {output_path}")
    except Exception as e:
